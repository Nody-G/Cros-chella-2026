...
export default function ChatPage() {
    ...
    if (typeof window !== "undefined" && "serviceWorker" in navigator && "PushManager" in window) {
      getPushSubscription().then((sub) => {
        if (!sub && Notification.permission !== "denied") {
          const dismissed = sessionStorage.getItem("cros-chella-dismissed-push-banner");
          if (!dismissed) {
  ...
  const handleEnablePush = async () => {
    if (!currentUserId) return;
    const success = await subscribeToPush(currentUserId);
    if (success) {
  ...
  const handleDismissBanner = () => {
    setShowPushBanner(false);
    sessionStorage.setItem("cros-chella-dismissed-push-banner", "true");
  };
  ...
  return (
    <main className="pb-20 min-h-screen flex flex-col">
      <div className="max-w-lg mx-auto w-full flex flex-col flex-1">
        ...
        {showPushBanner && (
          <div className="mx-4 mt-4 p-4 rounded-xl border border-primary/20 bg-primary/5 flex flex-col gap-3 relative overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            <div className="flex gap-3 items-start">
              ...
              <div className="flex-1 pr-6">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Ne rate aucun message important du festival. Reçois des alertes directes sur ton téléphone.
                </p>
              </div>
            </div>
